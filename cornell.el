...
(defun conv/cornell-init ()
  ...
  (if (not (f-this-file))
	...
	(split-window-below 32)
	(split-window-horizontally 120)
	(other-window 1)
	(setq conv/cornell-buffer-name-list (conv/cornell-split-file (buffer-name)))
	(find-file (concat (car conv/cornell-buffer-name-list) "-cues" (car (cdr conv/cornell-buffer-name-list))))
	(other-window 1)
	(find-file (concat (car conv/cornell-buffer-name-list) "-summary" (car (cdr conv/cornell-buffer-name-list))))
    (other-window 1)))
